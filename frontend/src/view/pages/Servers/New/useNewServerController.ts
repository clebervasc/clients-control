...
export function useNewServerController() {
  const { isNewServerModalOpen, closeNewServerModal } = useServer()
  const {
    register,
  ...
  const { isLoading, mutateAsync } = useMutation(serversService.create)
  ...
  const handleSubmit = hookFormSubmit(async (data) => {
    try {
      ...
      toast.success('Sucesso ao cadastrar o servidor!')

      reset()
      closeNewServerModal()
    } catch {
      toast.error('Erro ao cadastrar o servidor!')
    }
  })
